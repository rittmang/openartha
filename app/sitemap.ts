...
export default function sitemap(): MetadataRoute.Sitemap {
  ...
  const staticRoutes = ['', '/gita', '/gita/commentaries', '/sources', '/data', '/methodology'];
  return [
    ...
    ...publicCommentaryAuthors.flatMap((author) => [
      { url: `${base}/gita/commentaries/${author.id}`, changeFrequency: 'yearly' as const, priority: .5 },
      ...Array.from({ length: 18 }, (_, index) => ({ url: `${base}/gita/commentaries/${author.id}/${index + 1}`, changeFrequency: 'yearly' as const, priority: .4 })),
    ]),
  ];
}
